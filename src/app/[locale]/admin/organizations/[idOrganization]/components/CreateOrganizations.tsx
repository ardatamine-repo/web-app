...
import { createOrganization } from "@/lib/features/organizations";
...
export default function CrearOrganizacionModal({ onClose }: Props) {
  ...
  const initialState: NewOrganizationForm = {
    ...
    country: "Argentina",
    industry: "Tecnología",
    website: "https://testcreate.example.com",
    leader_uuid: id || "", // Evita error de tipo
    leader_info: "Juan Pérez, CEO",
    db_url_encrypted: "encrypted_db_url_example",
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (createOrganization.fulfilled.match(resultAction)) {
        setSuccess(true);
        setForm({ ...initialState, leader_uuid: id }); // aseguramos que el ID quede
      } else {
        const error =
